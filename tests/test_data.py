...
def test_airdens():
    ...
    assert np.allclose(test0,test_out[:,0]) & np.allclose(test1,test_out[:,1])
...
def test_satvap():
    test3 = satvap(Ta)
    test4 = satvap(Ta,Pa)
    assert np.allclose(test3,test_out[:,3]) & np.allclose(test4,test_out[:,4])
...
# Note that Matlab and Python versions have different tolerance thresholds for testing convergence of u10
